...
const sendEmail = async (to, templateName, data) => {
  try {
    ...
    logger.info(
      `Email sent: ${templateName} → ${to} (MessageId: ${info.messageId})`,
    );
    return { success: true, messageId: info.messageId };
  } catch (error) {
    logger.error(
      `Email send failed: ${templateName} → ${to}: ${error.message}`,
    );
...
const sendBulkEmail = async (recipients, templateName, data) => {
  ...
  logger.info(
    `Bulk email: ${successful} sent, ${failed} failed (template: ${templateName})`,
  );
