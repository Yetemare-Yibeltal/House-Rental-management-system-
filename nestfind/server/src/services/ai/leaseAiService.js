...
Return ONLY this JSON structure (no other text):
{
  ...
  "keyObligations": {
    "tenant": ["<obligation 1>", "<obligation 2>", "<obligation 3>", "<obligation 4>"],
    "landlord": ["<obligation 1>", "<obligation 2>", "<obligation 3>"]
  },
  "keyDates": [
    {
  ...
  "tenantRights": ["<right 1>", "<right 2>", "<right 3>"],
  "redFlags": ["<red flag if any>"],
  "overallAssessment": "<1-2 sentences: is this a fair contract for the tenant?>"
}`;

    const result = await aiService.sendMessage(prompt, "lease_explainer", {
      maxTokens: 2000,
    });
    ...
    if (!result.success || !result.content) {
      return {
        success: false,
        error:
          "AI lease explainer is temporarily unavailable. Please try again.",
      };
    }
    ...
    logger.info(
      `Lease explained for contract ${contractId} in ${responseTimeMs}ms`,
    );
...
Notice Period: ${terms.noticePeriodDays || 30} days notice required
Renewal Option: ${terms.renewalOption ? "Yes" : "No"}
...
CONTRACT BODY:
${contract.contractBody ? contract.contractBody.substring(0, 3000) : "Full contract body not available"}
`.trim();
};
...
const analyzeClause = async (clauseText, userId) => {
  try {
    if (!clauseText || clauseText.trim().length < 10) {
      return { success: false, error: "Please provide a clause to analyze" };
    }
  ...
  try {
    ...
    if (newTerms.monthlyRent !== originalTerms.monthlyRent) {
      ...
      changes.push({
        ...
        change:
          diff > 0
            ? `Increase of ETB ${diff.toLocaleString()}`
            : `Decrease of ETB ${Math.abs(diff).toLocaleString()}`,
